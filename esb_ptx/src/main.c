...
static void sample_handler(struct k_timer* timer) {
  static uint16_t samples[2] = {0, 1};

  esb_flush_tx();

  memcpy(tx_payload.data, samples, 4);

  volatile int err = esb_write_payload(&tx_payload);
  if (err) {
...
int main(void) {
  ...
  //  k_timer_start(&sample_timer, K_NSEC(20830), K_NSEC(20830));
